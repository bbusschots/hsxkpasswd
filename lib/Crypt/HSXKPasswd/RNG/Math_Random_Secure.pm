...
my $_CAN_MATH_RANDOM_SECURE = eval{
...
# utility variables
my $_CLASS = 'Crypt::HSXKPasswd::RNG::Math_Random_Secure';
...
sub new{
    ...
    unless($_CAN_MATH_RANDOM_SECURE){
        $_MAIN_CLASS->_error('This RNG module requires Math::Random::Secure, which is not installed');
    }
...
sub random_numbers{
    ...
    while($num_to_generate > 0){
        push @ans, Math::Random::Secure::rand();
        $num_to_generate--;
    }
